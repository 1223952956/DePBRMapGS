...
class SMPLX(SMPLH):
    ...
    def __init__(
        ...
        dtype=torch.float32,
        ext: str = "pkl",
        mapping_list: Optional[str] = None,
        **kwargs,
    ) -> None:
        ...
        if mapping_list is not None:
            self.mapping_list = torch.from_numpy(np.load(mapping_list)).cuda().long()
        data_struct = Struct(**model_data)
    ...
    def get_lbs_weights(self):
        return self.lbs_weights[self.mapping_list]

    @property
    def get_v_template(self):
        return self.v_template[self.mapping_list]

    @property
    def get_faces(self):
        return torch.from_numpy(np.load("no_eye_faces.npy")).long().cuda()
    ...
    def forward(
        ...
        **kwargs,
        ...
        if return_shaped:
            v_shaped = self.v_template + blend_shapes(betas, self.shapedirs)
        output = SMPLXOutput(
            vertices=vertices[:, self.mapping_list] if return_verts else None,
            joints=joints,
            betas=betas,
